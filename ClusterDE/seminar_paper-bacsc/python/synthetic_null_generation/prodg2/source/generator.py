...
class DataGenerator:
    ...
    def fit_marginals(self, X, marginal='auto', pval_cutoff=0.05):
        ...
        if marginal == 'auto':
            result = np.array([self.fit_auto(X.loc[i], pval_cutoff=pval_cutoff) for i in range(p)])
            params = np.array([item['params'] for item in result])
            models = np.array([item['model'] for item in result])
        elif marginal == 'zinb':
            params = np.array([self.fit_zinb(X.loc[i], pval_cutoff=pval_cutoff) for i in range(p)])
            models = np.full(p, 'zinb')
        elif marginal == 'nb':
            params = np.array([self.fit_nb(X.loc[i]) for i in range(p)])
            models = np.full(p, 'nb')
        elif marginal == 'poisson':
            params = np.array([[0.0, np.inf, np.mean(X.loc[:, i])] for i in range(p)])
